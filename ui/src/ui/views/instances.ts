  ...
  const scopesLabel =
    scopes.length > 0
      ? scopes.length > 3
        ...
        : t("instances.scopesList").replace("{list}", scopes.join(", "))
      : null;
  return html`
    <div class="list-item">
      <div class="list-main">
      ...
      <div class="list-meta">
        ...
        <div class="muted">${t("instances.lastInput").replace("{time}", lastInput)}</div>
        <div class="muted">${t("instances.reason").replace("{reason}", entry.reason ?? "")}</div>
      </div>
    </div>
  `;
}
